...
async def send_command(attack_type: str):
    ...
    try:
        if attack_type in LABEL_MAP:
            ...
            return {"status": "error", "details": "Geçersiz saldırı türü"}
        ...
        return {"status": "error", "details": "Makine 3'e baglanilamadi."}
...
async def receive_telemetry(data: TelemetryData):
    ...
    #gelen veri temiz ise delta hesapla ve dosyaya yaz 
    if data.type == "telemetry":
        delta_time = time.time() - data.paket_timestamp if data.paket_timestamp else 0.0
